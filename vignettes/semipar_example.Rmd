...
First, the function can be used to find the implied market size, $M$.
...
div_from_1[is.na(div_from_1)] <- 0
 ...
 find_M <- optimize(f = impliedshare_calc, lower = 50, upper = 7250,
    div_from = div_from_1, 
    quantities=out$provider_level$N_h, 
    focal=1,
    div_to=c(0,0,out$provider_level$div_from_5[1],0,0,0,0,0), 
...
Then, use the implied market size to determine the implied shares for each hospital in the market.

```{r}
 impliedshare_calc(M = M_implied, div_from = div_from_1, 
    quantities=out$provider_level$N_h, 
    focal=1,
    div_to=c(0,0,out$provider_level$div_from_5[1],0,0,0,0,0), 
    same_nest = c(3) )
